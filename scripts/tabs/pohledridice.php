...
$col  = ($l === 'en') ? "COALESCE(NULLIF(pohledridice_en, ''), pohledridice)" : "pohledridice";
$sql  = "SELECT $col AS pohledridice FROM texty WHERE linka = ?";
...
// 4) Výstup
$pohledHtml = $row['pohledridice'] ?? '';

if ($pohledHtml === '' || trim(strip_tags($pohledHtml)) === '') {
    echo "<p>" . ($lang['videocakana']) . "</p>";
} else {
    // Obsah z DB je HTML (typicky <iframe> YouTube + text) → ponecháme bez escapingu.
    echo "<div style='text-align:left'>{$pohledHtml}</div>";
}
