...
- `mapa-assets/data/*.json` — **generovaná data, commitují se a deployují**: `stops.json` (224 stanic), `routes.json` (51 linek; u asymetrických linek navíc `directions: [{headsign, stops}]` — seznamy zastávek po směrech, jinak jeden `stops`), `shapes.json` (geometrie tras, GeoJSON), `meta.json` (platnost feedu + `counts`), `timetable.json` (jízdní řád).
- `mapa-assets/data/timetable.json` — **generovaný** build_data.py: kompaktní JŘ pro klientské funkce *odjezdy v zastávce* a *poloha vozidel*. `services` = service_id → týdenní bitmaska po–ne + rozsah platnosti (z `calendar.txt`; `calendar_dates.txt` je u DPMLJ prázdný). `trips` = linka, mód (tram/bus), cíl (headsign), service_id a sled `[stopIdx, sekunda]` (stopIdx = index do `stops.json`; čas může být ≥86400 = spoj po půlnoci). Vozidla se zobrazují **přímo v zastávkách** (bez mezilehlé interpolace; vědomé zjednodušení — geometrie/`shape_dist` se nepoužívá): kolečko (tram #cc2900 / bus #007db3) + šipka k další zastávce; konečná bez šipky a +2 min po příjezdu, výchozí −2 min před odjezdem. `mapa.js` počítá „teď" v TZ Europe/Prague (i přes půlnoc přes službu předešlého dne), auto-refresh ~20 s, přepínač „Vozidla".
- `mapa-assets/data/legacy-routes.json` — **ručně udržovaný** (NE z build_data.py): neprovozní linky bez GTFS dat. Formát: `[{ "short_name":"50", "long_name":"…", "type":"bus|tram", "category":"historicke"?, "approximate":true?, "stops":["Název zastávky" | {"name":"…","lat":…,"lon":…}, …] }]`. `stops` slouží **jen k vykreslení trasy** (waypointy) — seznam zastávek se bere z DB. `category:"historicke"` = historická linka (filtr „Historické", barva #991f00, v legendě, v z-pořadí úplně dole); jinak „mimo provoz". `approximate:true` = ukáže poznámku „Trasa je přibližná." (jen linky s neznámými zastávkami, A-F/6-8). Na mapě se kreslí **čárkovaně** a defaultně skrytě (jen příslušný filtr / hover / focus).
...
- `mapa-assets/data/stops-history.json` — **ručně udržovaný**: `renamed` (starý název → dnešní GTFS název), `aliases` (jen jiný zápis téhož názvu), `vanished` (zaniklé zastávky se souřadnicemi). Používá **jen mapa** (`mapa.js`) — v DB seznamech legacy linek dělá staré/varianty názvů klikací a zaniklé zastávky přidává do seznamu „Zastávky" (kurzívou, klikací dle souřadnic). **Pozn.:** kurzívou se značí jen `vanished`; přejmenované existující zastávky jsou klikací bez označení. V přehledu se historie **neaplikuje**.
...
- **Hotovo navíc:** seznam zastávek po směrech (jednosměrné zastávky/závleky); jízdní řády z GTFS — odjezdy v zastávce (následující hodina) + poloha vozidel dle JŘ (`timetable.json`).
- **Otevřené možnosti (dle priorit uživatele):** přepnutí podkladu na keyed providera před produkcí; zobrazit jen aktuálně provozované linky; poloha vozidel vedená po trase (shapes + `shape_dist`) místo skoků mezi zastávkami; později vyhledávání spojení přes routing API (`pttsoftware.eu`, vyžaduje backend).
